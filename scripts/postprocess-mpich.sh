...
function plotoverallplots ()
{
    ...
    then
        ...
        then
            ...
            gnuplot -e "set term png font \"/usr/share/fonts/dejavu/DejaVuSans.ttf,30\" size 2048,2048 linewidth 2; set output \"""$timestamp"".timegraphwithweights.png\"; set size 1,1; set xtics rotate 250; set multiplot; set title \"Evolution of firing rates\"; set xlabel \"Time (s)\"; set ylabel \"Firing rates (Hz)\"; set origin 0.0, 0.5; set size 1,0.5; plot \"""$timestamp"".e.rate-allmerged\" u 1 : ((sum [col=2:""$numberofcolsE""] column(col))/""$numberofcolsE"") lw 3 lc rgb \"blue\" t \"Average Excitatory firing rate (Hz)\" with lines, \"""$timestamp"".i.rate-allmerged\" u 1 : ((sum [col=2:""$numberofcolsI""] column(col))/""$numberofcolsI"") lw 3 lc rgb \"red\" t \"Average Inhibitory Firing rate (Hz)\" with lines; set origin 0.0,0.0; set size 1,0.5; set ylabel \"Synaptic weight (times gleak)\"; set title \"Evolution of synapses\"; plot \"""$timestamp"".ie_stdp.weightinfo-allmerged\" u 1 : ((sum [col=2:""$numberofcolsWeightFiles""] column(col))/""$totalConnectionsIE"") lw 3 lc rgb \"brown\" t \"Average IE (plastic)\" with lines, \"""$timestamp"".ii_static.weightinfo-allmerged\" u 1 : ((sum [col=2:""$numberofcolsWeightFiles""] column(col))/""$totalConnectionsII"")  t \"Average II (static)\" lw 3 with lines, \"""$timestamp"".ee_static.weightinfo-allmerged\" u 1 : ((sum [col=2:""$numberofcolsWeightFiles""] column(col))/""$totalConnectionsEE"")  t \"Average EE (static)\" lw 3 with lines, \"""$timestamp"".ei_static.weightinfo-allmerged\" u 1 : ((sum [col=2:""$numberofcolsWeightFiles""] column(col))/""$totalConnectionsEI"") t \"Average EI (static)\" lw 3 with lines, \"""$timestamp"".exte_static.weightinfo-allmerged\" u 1 : ((sum [col=2:""$numberofcolsWeightFiles""] column(col))/""$totalConnectionsEXTE"") t \"Average EXTE (static)\" lw 3 with lines" &
