...
# Other functions
mainworkfunction <- function (time, spikeDataE, spikeDataI, output) 
{
...
#timeToSnapshotStr <- strsplit(args[2:length(args)]," ")
#timeToSnapshot <- lapply(timeToSnapshotStr,as.numeric)
#print(timeToSnapshot)
#q()
...
for (time in timeToSnapshot) {
    ...
    if (!is.na(time))
    {
        ...
        mainworkfunction(as.numeric(time), spikeDataE, spikeDataI, outputFile)
    }
}
